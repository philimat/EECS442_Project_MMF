...
if __name__ == '__main__':
    ...
    for lidar_file in tqdm(glob(os.path.join(pc_dir,'*.bin'))):
        ...
        if i == 0:
            ...
            grid = BEVOccupancyGrid(scan.points, 448, 512, 32,side_range=(-40,40),fwd_range=(0,70.4),height_range=(-3,1), filter_external=True)
            grid.compute()
            ...
            Y, X = np.meshgrid(grid.midsegments[0],grid.midsegments[1])
            XX = X.flatten()
            YY = Y.flatten()
